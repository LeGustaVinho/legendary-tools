...
namespace LegendaryTools
{
    ...
    [CustomEditor(typeof(PropertySync))]
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!serializedObject.isEditingMultipleObjects)
            {
                if (pb.source != null && pb.target != null && pb.source.GetPropertyType() != pb.target.GetPropertyType())
                {
                    ...
                    {
                        EditorGUILayout.HelpBox("Bi-Directional updates require both Source and Target to reference values of the same type.", UnityEditor.MessageType.Error);
                    }
                    else if (pb.direction == PropertySync.Direction.SourceUpdatesTarget)
                    {
                        if (!PropertyBindingReference.Convert(pb.source.Get(), pb.target.GetPropertyType()))
                        {
                            EditorGUILayout.HelpBox("Unable to convert " + pb.source.GetPropertyType() + " to " + pb.target.GetPropertyType(), UnityEditor.MessageType.Error);
                        }
                    }
                    else if (!PropertyBindingReference.Convert(pb.target.Get(), pb.source.GetPropertyType()))
                    {
                        EditorGUILayout.HelpBox("Unable to convert " + pb.target.GetPropertyType() + " to " + pb.source.GetPropertyType(), UnityEditor.MessageType.Error);
                    }
                }
            }
