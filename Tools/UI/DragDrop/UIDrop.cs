...
namespace LegendaryTools.UI
{
    ...
    public class UIDrop : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public bool SnapTransformPosition = false;
        [Tooltip("Parent the object with slot(container)")]
        public bool ParentWithContainer = false;
        [Tooltip("Restore object original anchors and pivot")]
        public bool RestoreAnchorAndPivot = false;
        ...
        public void OnDrop(PointerEventData eventData)
        {
            if (!UIDrag.Dragging[eventData.pointerId].OnEndDrag)
                UIDrag.Dragging[eventData.pointerId].Object.GetComponent<UIDrag>().OnEndDrag(eventData); //Ensure execution order OnBeginDrag -> OnDrag -> OnEndDrag -> OnDrop

            if (UIDrag.Dragging[eventData.pointerId].OnDrop) return;
        ...
        {
            if (RectTransformUtility.RectangleContainsScreenPoint(transform as RectTransform, eventData.position, eventData.enterEventCamera)) //to make sure that the object will be inside a slot
            {
                if (SwapItems)
                    return true;
